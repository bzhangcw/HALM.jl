...
if bool_setup
    ...
    include("problems/prob_regls_tiny.jl")
end
...
plt_vs = [(v[1], v[2]) for v in (vs .* sign.(ts))]
angles = collect(range(0, 2π, length=100))
unit_circle = [(cos(θ), sin(θ)) for θ in angles]
plot(unit_circle, aspect_ratio=:equal)
scatter!(plt_vs, aspect_ratio=:equal)
xlims!(-0.1, 0.1)
ylims!(-0.1, 0.1)
